...
p = ggplot(myb_vs_hpv,aes( x = hpv_positive, y = MYB)) +
  theme_minimal()+
  stat_summary(fun.data = function(x) data.frame(y=max(x)*1.1, label = paste("Mean=",round(mean(x),digits = 2))), geom="text") +ylab("average MYB")+ggtitle("all patients") +
  stat_pvalue_manual(stat.test,y.position = 2.6, label = "Wilcox, p = {p}",remove.bracket = F)+
