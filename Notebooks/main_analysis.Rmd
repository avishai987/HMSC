...
```{r}
hmsc_cancer_cells = readRDS("./Data_out/HMSC_cells_preprocess_2024-05-12/acc1_cancer_cells_2500features_integrated_V5.RDS")
hmsc_acc_pri_cancer = readRDS("./Data_out/build_datasets_2024-05-12/hmsc_acc_pri_cancer_processed.RDS")
...
plt %+% (plt$data %>% arrange(signature))
plt+ scale_y_discrete(limits = arrange(plt$data,signature,significance)[,"label"])

plt1 = plt$up+ aes(color=nes)+ggtitle("Up in HMSC")
plt2 = plt$dn+ aes(color=nes)+ggtitle("Down in HMSC")

p = ggarrange(plt1,plt2,common.legend = T,nrow = 2,legend = "right")
p
```
...
```{r}
pdf(file = "./Figures/ACC_vs_HMSC_GSEA.pdf",width = 12,height = 4)
p
dev.off()
```
...
```{r}
top_genes = acc_deg %>% filter(.$fdr<0.05) %>% arrange(dplyr::desc(.$avg_log2FC)) %>% head(10) %>% rownames()
plt = volcano_plot.2(de_genes = acc_deg,fc_cutoff = 1.3, fdr_cutoff = 0.1,show_gene_names = intersect( # OS HNSC 0.032 CECS 0.065
 ...
 ),ident1 = "HPV33 positive",ident2 = "HPV33 negative",top_genes_text = 0) +ggtitle("Differential Gene Expression by HPV Status
")
plt
```
```{r}
acc_deg %>% filter(.$fdr<0.05) %>% filter(.$avg_log2FC>log2(1.3)) %>%  arrange(dplyr::desc(.$avg_log2FC)) %>% head(10) %>% rownames() %>% cat(sep = "\n")
```
