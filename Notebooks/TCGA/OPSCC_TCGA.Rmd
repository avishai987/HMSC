...
hmsc_signatue = rownames(hmsc_deg[hmsc_deg$fdr<0.05 & hmsc_deg$avg_log2FC>log2(1.5),])

avg_log2FC_cutoff = log2(1.2)
fdr_cutoff = 0.05
...
gene_status = set_clinical_data(clin_data = clinical_data,genes = "MYC",tpm_data_frame = HNSC_tpm,stratify = params$stratify)
gene_status$patient = rownames(gene_status)
gene_status = inner_join(x = gene_status,y = clinical_data[,c("submitter_id","hpv_status")],by = c("patient"="submitter_id"))
...
library(rstatix)
stat.test <- gene_status %>%
  t_test(MYC ~ hpv_status) %>%
  add_significance()
stat.test


bxp <- ggboxplot(gene_status, x = "hpv_status", y = "MYC", fill = "#00AFBB")+
  geom_jitter()
stat.test <- stat.test %>% add_xy_position(x = "hpv_status")
bxp + 
  stat_pvalue_manual(stat.test,  label = "T-test, p = {p}") 
