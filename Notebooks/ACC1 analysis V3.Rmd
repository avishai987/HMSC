...
```{r}
cnv_subtypes = cnv_subtypes %>% mutate(plate = case_when(
         rownames(cnv_subtypes)[row_number()] %in% plate3 ~ "plate_3",
         rownames(cnv_subtypes)[row_number()] %in% plate2 ~ "plate_2",
         TRUE  ~ "ok"))

# creat colours for each group
annoCol <- list(plate = c(plate_3 = "green",plate_2 = "red"),cnv.cluster =c("1"="green","2" = "red"))
