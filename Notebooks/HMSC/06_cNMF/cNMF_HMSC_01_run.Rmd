...
```{python}
os.mkdir(r.output_dir_path + "preprocess_output")
p = Preprocess(random_seed=14)
adata = r.adata
(adata_c, adata_tp10k, hvgs) = p.preprocess_for_cnmf(adata, harmony_vars='orig.ident', n_top_rna_genes = 2000,max_scaled_thresh = 50, quantile_thresh = .9999,theta = 2, makeplots=True,save_output_base=r.output_dir_path + "preprocess_output/hmsc")
```



```{python}
cnmf_obj_corrected = cNMF(output_dir=r.output_dir_path , name='HMSC_BatchCorrected_NMF')
cnmf_obj_corrected.prepare(
  counts_fn=r.output_dir_path + 'preprocess_output/hmsc.Corrected.HVG.Varnorm.h5ad',
  tpm_fn=r.output_dir_path + 'preprocess_output/hmsc.TP10K.h5ad', 
  genes_file=r.output_dir_path + 'preprocess_output/hmsc.Corrected.HVGs.txt',
  components=np.arange(3,11), n_iter=20, seed=14, num_highvar_genes=2000)
```
...
```{python}
import pickle
with open(r.output_dir_path + "HMSC_BatchCorrected_NMF/" + "cnmf_obj.pkl", 'wb') as file: 
