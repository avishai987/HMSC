...
# load MYB targets
library("readxl")
myb_tergets <- read_excel("./Input_data/MYB_tergets_PMC4767593_sup_table_4.xlsx",col_names = T) %>% plyr::rename( replace = c("Total grade 2 MYB signal" = "MYB_signal")) %>%  filter (MYB_signal>25) %>%  pull(Gene)
```
...
# plate2 = hmsc_cancer_cells[,hmsc_cancer_cells$plate == "ACC.plate2"]
# plate3 = hmsc_cancer_cells[,hmsc_cancer_cells$plate == "ACC1.P3"]
# 
# plate2 = FindVariableFeatures(plate2,nfeatures = 15000,assay = "RNA")
# plate3 = FindVariableFeatures(plate3,nfeatures = 15000,assay = "RNA")
# 
# which(VariableFeatures(plate2,assay = "RNA")  %in% "MYB")
# which(VariableFeatures(plate3,assay = "RNA")  %in% "MYB")
# 
# combine_gene = c(VariableFeatures(plate2), VariableFeatures(plate3))
# 
# xor_genes = combine_gene[! combine_gene %in%  intersect(VariableFeatures(plate2), VariableFeatures(plate3))]
...
genesets[["myb_targets"]] = myb_tergets
...
```{r}
write.csv(x  = acc_deg,file = paste0(params$data_out_dir,"HMSC_hpv_deg_df.csv"),row.names = T)
```
