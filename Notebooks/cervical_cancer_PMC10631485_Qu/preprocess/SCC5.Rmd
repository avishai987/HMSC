...
```{r}
saveRDS(object = DietSeurat(SCC5_cancer),file = "./Data/cervical_cancer_data/Qiu et al/SCC5/SCC5_cancer_processed.RDS")
```
...
p1 = ggplot(myb_vs_hpv,aes( x = hpv_positive, y = MYB,, fill=hpv_positive))+ 
  ...
  stat_summary(fun.data = function(x) data.frame(y=max(x)*1.2, label = paste("Mean=",round(mean(x),digits = 2))), geom="text") +ylab("MYB")+ggtitle("SCC5") +
...
df = rbind(AverageExpression(object = SCC5_cancer,features = "MYB",group.by = "hpv_positive")[[1]],
AverageExpression(object = SCC4_cancer,features = "MYB",group.by = "hpv_positive")[[1]],
AverageExpression(object = SCC3_cancer,features = "MYB",group.by = "hpv_positive")[[1]],
AverageExpression(object = SCC2_cancer,features = "MYB",group.by = "hpv_positive")[[1]]) %>% as.data.frame()

df$patient = c("SCC5","SCC4","SCC3","SCC2")
df = reshape2::melt(df,value.name = "MYB")
 ggboxplot(df, x = "variable", y ="MYB",palette = "jco",
            add = "jitter") + theme_minimal()+
  stat_compare_means(method = "t.test",comparisons = list(c("positive","negative")))+
  stat_summary(fun.data = function(x) data.frame(y=max(x)*1.2, label = paste("Mean=",round(mean(x),digits = 2))), geom="text")
