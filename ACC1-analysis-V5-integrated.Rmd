...
features = VariableFeatures(acc1_cancer_cells) %>% intersect(all_coding_genes$hgnc_symbol)
acc_deg <- FindMarkers(acc1_cancer_cells, ident.1 = "positive",ident.2 = "negative",features = features,densify = T,logfc.threshold = 0.25,min.diff.pct = 0)
...
ggboxplot(
  ...
  fun.data = function(x)
    data.frame(y = max(x) * 1.2, label = round(mean(x), digits = 2)),
  geom = "text",
  aes(group = hpv33_positive),
...
```{r}
myb_vs_hpv = FetchData(object = acc1_cancer_cells,vars = c("hpv33_positive",c("JAG1","MYB"))) 
myb_vs_hpv$hpv33_positive = paste("HPV33",myb_vs_hpv$hpv33_positiv)
df = melt(myb_vs_hpv)
